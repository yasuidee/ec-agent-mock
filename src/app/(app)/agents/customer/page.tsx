...
const sentimentBorder: Record<string, string> = {
  positive: 'border-l-4 border-green-400',
  neutral: 'border-l-4 border-slate-300',
  negative: 'border-l-4 border-red-400',
};
...
export default function CustomerAgentPage() {
  ...
  const selectClass = 'rounded-md border border-input bg-background px-3 py-2 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-900/30';
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <PageHeader
        title="顧客対応AI"
        description="レビュー・問い合わせへのAI返信アシストと顧客フィードバック分析"
      />
      ...
      <Tabs defaultValue="reviews">
        <TabsList>
          <TabsTrigger value="reviews">💬 レビュー返信</TabsTrigger>
          <TabsTrigger value="inquiries">📩 問い合わせ返信</TabsTrigger>
          <TabsTrigger value="analysis">📊 フィードバック分析</TabsTrigger>
        </TabsList>

        {/* ══ TAB 1: レビュー返信 ══ */}
        <TabsContent value="reviews">
          {/* Filter bar */}
          <div className="bg-white border rounded-xl p-4 mb-4 flex items-center gap-3 flex-wrap">
          ...
          <div className="space-y-4">
            {filteredReviews.map(review => {
              ...
              return (
                <div key={review.id} className={`bg-white border rounded-xl p-5 ${sentimentBorder[review.sentiment]}`}>
                        ...
                        {hasGenerated && !isLoading && (
                          <div>
                            <textarea
                              ...
                              rows={4}
                              className="w-full border rounded-lg px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-900/30 resize-none"
                            />
                            <div className="flex gap-2 mt-2">
                              <button
                                onClick={() => applyReply(review.id, 'review')}
                                className="bg-blue-900 text-white text-sm px-4 py-2 rounded hover:bg-blue-800 transition-colors"
                              >
                                この返信を使う
                              </button>
                              <button
                                onClick={() => generateReply(review.id, 'review')}
                                className="border text-sm px-3 py-2 rounded text-slate-600 hover:bg-slate-50 transition-colors"
                              >
                                再生成する
                              </button>
                            </div>
        ...
        <TabsContent value="inquiries">
          ...
          <div className="space-y-4">
            {filteredInquiries.map(inquiry => {
              ...
              return (
                <div key={inquiry.id} className={`bg-white border rounded-xl p-5 ${priorityBorder[inquiry.priority]}`}>
                  <div className="flex items-start justify-between">
                    <div className="flex gap-2 flex-wrap">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-xl p-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                {Object.entries(positiveTagCounts).sort((a, b) => b[1] - a[1]).map(([tag, count]) => (
                  <div key={tag} className="bg-white border border-green-200 rounded-lg p-4">
            ...
            <div className="bg-white border rounded-xl p-6">
              ...
              <button
                onClick={generateFaq}
                disabled={faqLoading}
                className="bg-blue-900 text-white px-6 py-3 rounded-lg text-sm font-medium hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
              >
                {faqLoading ? (
                  ...
                  {generatedFaqs.map((faq, i) => (
                    <div key={i} className="border rounded-lg p-4">
                      ...
                      <button
                        ...
                        className="border text-sm px-3 py-2 rounded mt-3 text-slate-600 hover:bg-slate-50 transition-colors"
                      >
                        商品ページのFAQに追加する
                      </button>
                    </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
              <h2 className="font-semibold text-slate-900 mb-4">🤖 AIからの改善提案</h2>
              <button
              ...
              {customerReport && (
                <div className="mt-4">
                  <p className="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{customerReport}</p>
                  <div className="grid grid-cols-3 gap-3 mt-4">
                    <button onClick={() => router.push('/agents/build')} className="border rounded-lg p-3 text-sm text-slate-700 hover:bg-white transition-colors text-left">
                      🏗️ 商品ページを改善する →
                    </button>
                    <button onClick={() => router.push('/agents/marketing')} className="border rounded-lg p-3 text-sm text-slate-700 hover:bg-white transition-colors text-left">
                      📣 広告・集客を見直す →
                    </button>
                    <button onClick={() => router.push('/agents/inventory')} className="border rounded-lg p-3 text-sm text-slate-700 hover:bg-white transition-colors text-left">
                      📦 在庫・品質を見直す →
                    </button>
                  </div>
                </div>
